      ...
      .then((response)=>{
        if(response.data.error) {
         console.log(response.data.error);
            ...
            const commentToAdd = {commentBody:newComment, id:response.data.id}  
    ...
    <div className='postPage'>
        <div className='leftSide'>
          <div className='post' id='individual'>
            ...
            <div className='postText'onClick={()=>{editPost('body')}}>{postObject.postText}</div>
            <div className='footer'>
              {postObject.username} <button onClick={()=>deletePost(postObject.id)}>Delete Post</button>
              </div>
          </div>
        </div>
        <div className='rightSide'>
          <div className='addCommentContainer'>
            <input type='text' placeholder='Add Comment..' value={newComment} autoComplete='off'onChange={(event)=>{setNewComment(event.target.value)}}/>
            <button onClick={addComment}>Add Comment </button>
          </div>
          <div className='listOfComments'>
            {comments.map((comment , key)=>{
              return <div key={key} className='comment'>{comment.commentBody}
              <button onClick={() =>{deleteComment(comment.id)}}>X</button></div>
            })}
          </div>
        </div>
      </div>
